...
由于声明为static的变量只被初始化一次，因为它们在单独的静态存储中分配了空间，因此**类中的静态变量由对象共享。**对于**不同的对象，不能有相同静态变量的多个副本**。也是因为这个原因，**静态变量不能使用构造函数初始化**。
...
您可以在上面的程序中看到我们已经尝试为多个对象创建静态变量i的多个副本。但这并没有发生。因此，<font color='red'>类中的静态变量应由用户使用类外的类名和范围解析运算符显式初始化</font>，如下所示：
...
就像类中的静态数据成员或静态变量一样，**静态成员函数也不依赖于类的对象**。我们被**允许使用对象和'.'来调用静态成员函数。但建议使用类名和范围解析运算符调用静态成员**（也就是`class::func()`）。
...
- **静态类成员变量**不能使用构造函数初始化，也不能在定义时指定初值，只能在类外使用类名` int Apple::num=1;`这样赋值。
- 静态类函数**允许使用对象和'.'来调用静态成员函数。但建议使用类名和范围解析运算符调用静态成员**。静态类成员函数<font color='red'>只能访问类中静态成员变量和静态成员函数，不能访问类的非静态成员变量或成员函数。</font>
